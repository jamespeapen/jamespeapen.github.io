...
<<<<<<< HEAD
frame format, or a vector of region string could not be used directly.

I didn't want to use *Rhtslib* since it tends to update very slowly and I wanted a
more updated *htslib*. I was also unsure if having a Bioconductor dependency would
be hard to use if I submitted the package to CRAN. At first I just used a nix
environment and used `pkg-config` to pass *htslib* flags to the `Makevars` file
to compile and install the package. This was temporary since the flags can be
different depending on the system and its *htslib* installation.
=======
